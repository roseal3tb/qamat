...
export function Hero() {
  ...
  const featured = useOpenPrograms()[0];
    ...
    <section id="hero" ref={ref} className="pt-24 md:pt-28">
      <div className="container-q">
        <div className="qamat-hero relative overflow-hidden">
          ...
          >
            {/* شارة البرنامج المفتوح — تختفي تلقائيًا بعد إغلاق التسجيل */}
            {featured && (
              <motion.div
              ...
              >
                <Link
                  to="/programs"
                  className="group inline-flex items-center gap-2 rounded-full border border-[rgba(201,171,126,0.45)] bg-[rgba(242,237,226,0.06)] py-1 ps-1 pe-3.5 text-[0.72rem] backdrop-blur-sm transition-colors duration-300 hover:border-accent hover:bg-[rgba(242,237,226,0.1)] sm:text-xs"
                >
                  <span className="rounded-full bg-accent px-2.5 py-1 text-[0.65rem] font-semibold text-accent-foreground">
                    التسجيل مفتوح
                  </span>
                  <span className="font-medium">برنامج {featured.name}</span>
                  <ArrowLeft
                    aria-hidden
            ...
            <motion.a
              ...
              className="mt-12 flex flex-col items-center gap-2 text-xs text-muted-foreground transition-colors hover:text-accent-strong"
            >
              اكتشف
              <motion.span
                aria-hidden
